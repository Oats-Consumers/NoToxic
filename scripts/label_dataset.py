...
def load_unlabeled_dataset(path, count):
    with open(path, "r", encoding="utf-8") as f:
        return [json.loads(line) for _, line in zip(range(count), f)]

def save_labeled_dataset(path, entries):
...
if __name__ == "__main__":
    unlabeled = load_unlabeled_dataset(UNLABELED_PATH, NUM_ENTRIES)
    labeled = label_entries(unlabeled)
    save_labeled_dataset(LABELED_PATH, labeled)
    print(f"\n✅ Labeled {len(labeled)} entries. Saved to {LABELED_PATH}.")
